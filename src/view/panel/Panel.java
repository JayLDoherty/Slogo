...
public class Panel extends CollapsibleView {
	 ...
	 */
	public Panel(Workspace workspace, int index, TurtleDisplay turtleDisplay) {
		super(workspace.getPane(), index, Direction.RIGHT, true);
		this.workspace = workspace;
		createSubviews(turtleDisplay);
		setup();
	}
	 ...
	 */
	private void createSubviews(TurtleDisplay turtleDisplay) {
		subviewTitles = new ArrayList<String>() {
			private static final long serialVersionUID = 1L;
			{
				...
				add(workspace.getController().getResources().getString("ImageSelectorTitle"));
			}
		};
		subviews = new ArrayList<Node>() {
			private static final long serialVersionUID = 1L;
			{
				...
				add(new ImageSelectorView(workspace.getController(), turtleDisplay));
			}
		};
	}
